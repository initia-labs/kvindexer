...
type MoveNftSubmodule struct {
	...
	collectionMap      *collections.Map[sdk.AccAddress, types.IndexedCollection]
	collectionOwnerMap *collections.Map[collections.Pair[sdk.AccAddress, sdk.AccAddress], uint64]
	tokenMap           *collections.IndexedMap[collections.Pair[sdk.AccAddress, string], types.IndexedToken, TokenIndex]
...
func NewMoveNftSubmodule(
	...
	collectionMap, err := collection.AddMap(indexerKeeper, collectionsPrefix, "collections", sdk.AccAddressKey, codec.CollValue[types.IndexedCollection](cdc))
	if err != nil {
		return nil, err
	}
	...
	tokenMap, err := collection.AddIndexedMap(indexerKeeper, tokensPrefix, "tokens", collections.PairKeyCodec(sdk.AccAddressKey, collections.StringKey), codec.CollValue[types.IndexedToken](cdc), newTokensIndex(ac, indexerKeeper))
	if err != nil {
		return nil, err
	}
